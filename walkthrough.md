...
Hướng dẫn chi tiết về logic hoạt động của từng thành phần trong DRIFT autopilot trading system.
...
## 12. Autopilot Philosophy

DRIFT được thiết kế theo triết lý **"let it run"**:

### Farm Mode — Always Active
- Không bao giờ skip entry
- Luôn tìm cách vào lệnh (signal → MM bias → alternating)
- Tự động điều chỉnh size dựa trên confidence và performance
- Dynamic TP thích ứng với spread thực tế
- Cooldown ngắn để maximize frequency

### Trade Mode — Smart Filtering
- Chỉ vào khi có edge rõ ràng
- Multi-layer filtering: regime → chop → fake breakout → confidence
- Adaptive cooldown scale với market conditions
- Không có time pressure — để trade chạy đến TP/SL

### Self-Learning
- Signal weights tự điều chỉnh mỗi 10 trades
- Confidence calibration dựa trên historical win rate
- Fill rate feedback điều chỉnh execution offset
- Performance tracking scale position size

### Zero-Maintenance
- 70+ config parameters runtime tunable
- Graceful shutdown với position cleanup
- Telegram remote control
- Real-time dashboard monitoring
- Property-based testing đảm bảo correctness

**Kết quả**: Một trading system hoàn toàn tự động, tự học, tự tối ưu — bạn chỉ cần setup một lần, sau đó để nó chạy.