...
DRIFT là autopilot trading bot cho BTC perpetual futures. Nó tự động generate volume, thực thi chiến lược, và tự học — trong khi bạn không cần làm gì.

Hỗ trợ 3 sàn: **SoDEX**, **Dango Exchange**, và **Decibel**.
...
Momentum score từ 5m candles với **adaptive weights** (tự điều chỉnh mỗi 10 trades). Hệ thống tích hợp **SIDEWAY range intelligence** trực tiếp vào điểm số:
...
Khi LLM không phản hồi trong vùng SIDEWAY, vị trí trong range trở thành tín hiệu chính:
...
DRIFT hỗ trợ 3 sàn qua interface chung `ExchangeAdapter`.
